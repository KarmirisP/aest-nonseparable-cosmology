...
\begin{abstract}
We test a single-metric relativistic modified-gravity cosmology in which the particle cold-dark-matter density and the bare cosmological constant are fixed to zero. The theory combines an aether scalar--tensor sector, a current-linked nonseparable scalar interaction, and an auxiliary Type-II minimally modified gravity sector for late-time acceleration. The realization tested here is the cyclic, nonlinear realization of this framework rather than the earlier linearized realization. The frozen implementation is compared with a matched flat-$\LCDM$ control using the same Planck 2018 high-$\ell$ TTTEEE-lite, low-$\ell$ TT, low-$\ell$ EE, Planck lensing, DESI DR2 BAO, Pantheon+, and SH0ES Gaussian likelihood terms. Both Markov chains satisfy the common convergence requirement, with $R-1=0.00691$ for modified gravity and $0.00729$ for $\LCDM$. The lowest sampled independent seven-likelihood totals are $\chi^2_{\rm MG}=2441.26650$ and $\chi^2_{\LCDM}=2446.93250$, giving $\Delta\chi^2=-5.66600$. With seven fitted cosmological coordinates for modified gravity and six for $\LCDM$, $\Delta\mathrm{AIC}=-3.66600$ and, for the stated bookkeeping convention $N=2393$, $\Delta\mathrm{BIC}=+2.11431$. The modified-gravity posterior gives $H_0=68.672\pm0.305\,{\rm km\,s^{-1}\,Mpc^{-1}}$, compared with $68.835\pm0.273$ for $\LCDM$. The likelihood improvement is concentrated in Planck high-$\ell$, Pantheon+, and DESI DR2, while the SH0ES contribution changes by $+1.452$ in $\Delta\chi^2$. These results establish that the tested zero-particle-CDM realization is statistically competitive with the matched $\LCDM$ control on this likelihood stack. Complete analytic and numerical derivations used by the implementation are collected in the appendices.
\end{abstract}
...
The cosmological construction studied here is the cyclic, nonlinear realization of the framework. References below to linear scalar perturbations and linear matter spectra refer to the perturbative treatment used to compute observables and do not characterize the underlying model as a linear theory.
...
\label{eq:shoes}
\end{equation}
The stored chain values at the minimum-total points are reproduced by Eq.~\eqref{eq:shoes}: $16.0901$ for the modified-gravity point $H_0=68.8683$ and $14.6382$ for the $\LCDM$ point $H_0=69.0610$.
...
\end{align}
with a common stopping threshold $R-1<0.01$. The comparison uses the seven independent likelihood columns stored in the converged chains.
...
The lowest sampled independent seven-likelihood totals are
\begin{align}
\chi^2_{\rm MG}&=2441.2664987,\\
\chi^2_{\LCDM}&=2446.9324958,
\end{align}
so
\begin{equation}
\Delta\chi^2\equiv\chi^2_{\rm MG}-\chi^2_{\LCDM}=-5.6659971.
\end{equation}
The corresponding information-criterion differences are
\begin{align}
\Delta\mathrm{AIC}&=-3.6659971,\\
\Delta\mathrm{BIC}&=+2.1143060.
\end{align}
The maximum sampled likelihood and AIC favor the modified-gravity realization, whereas the stated BIC convention gives a mild preference to the lower-dimensional $\LCDM$ control.

\begin{table}[htbp]
\caption{Independent likelihood contributions at the lowest sampled total in each converged chain. Negative $\Delta\chi^2_i$ favors modified gravity.}\label{tab:likes}
\begin{ruledtabular}
\begin{tabular}{lrrr}
Likelihood & MG & $\LCDM$ & $\Delta\chi^2_i$\\\hline
Planck high-$\ell$ & 583.3074 & 586.8745 & -3.5670\\
Planck lowTT & 22.6653 & 22.7313 & -0.0661\\
Planck lowEE & 395.8339 & 396.1093 & -0.2754\\
Planck lensing & 8.7693 & 8.8027 & -0.0334\\
DESI DR2 & 9.2113 & 10.4194 & -1.2081\\
Pantheon+ & 1405.3892 & 1407.3572 & -1.9680\\
SH0ES & 16.0901 & 14.6382 & +1.4519\\\hline
Total & 2441.2665 & 2446.9325 & -5.6660\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
The largest contributions to the improvement are Planck high-$\ell$ ($-3.567$), Pantheon+ ($-1.968$), and DESI DR2 ($-1.208$). The SH0ES term contributes $+1.452$, so the net likelihood gain is generated by the CMB, BAO, and supernova sectors rather than by the local-distance-ladder term.
...
\end{align}
The minimum-$\chi^2$ sampled values are $68.8683$ and $69.0610$, respectively.
...
A fixed-coordinate profile, in which $H_0$ alone was varied from 66 to 76 while the remaining coordinates were held at the lowest sampled point, rises sharply away from the minimum. At the upper sampling boundary $H_0=70.5$, the profile increases by $\Delta\chi^2=331.03$ for modified gravity and $258.26$ for $\LCDM$ relative to the corresponding replay point. The profile minimum coincides with the chain-minimum $H_0$ for both models. These diagnostics show that the reported posterior region is well separated from the imposed $H_0$ boundaries.

\section{Context among recent cosmological extensions}\label{sec:context}
The present $\Delta\chi^2=-5.666$ is a matched result for one specific seven-likelihood stack. Recent early-dark-energy analyses using different CMB combinations and larger parameter spaces have reported substantially larger raw likelihood improvements when SH0ES is included \cite{Poulin2025EDE}. DESI DR2 analyses of the two-parameter $w_0w_a$ extension likewise find a preference whose size depends on the supernova sample and statistical treatment \cite{DESIDR2II2025,DESIDR2Extended2025,Ormondroyd2025,WangMota2025}. Other recent analyses examine phantom-crossing or sign-switch acceleration histories in related late-time data combinations \cite{Pang2025DDE,Dixit2026SignSwitch}.
...
with
\begin{equation}
\Delta\mathrm{AIC}=-3.6660,\qquad \Delta\mathrm{BIC}=+2.1143
\end{equation}
...
The seven independent likelihood contributions are summed directly from the stored chain columns. Aggregate columns are excluded to prevent double counting. At the modified-gravity minimum the contributions are
\begin{align}
...
\begin{align}
\chi^2_{\LCDM}={}&586.874460+22.731346+396.109270\nonumber\\
&+8.8026918+10.4193730+1407.357200\nonumber\\
&+14.638155=2446.9324958.
\end{align}
...
A fresh runtime replay at the stored minimum coordinates gave $\chi^2=2441.26096$ for modified gravity and $2447.10292$ for $\LCDM$. The release therefore preserves both the original chain likelihood columns and the replay audit. The matched comparison in Table~\ref{tab:likes} uses the likelihood values evaluated and stored by the converged production chains, identically for both models.
...
The fixed-coordinate profile was evaluated at 41 points from $H_0=66$ to $76\,{\rm km\,s^{-1}\,Mpc^{-1}}$. The profile minima occur at the production-chain minimum values. At $H_0=70.5$, the increases relative to the replay minima are $331.031$ and $258.257$ in $\chi^2$ for modified gravity and $\LCDM$; at $H_0=66$, the increases are $1016.881$ and $1191.094$. These values provide a direct diagnostic that the sampled posterior region is not accumulated against either $H_0$ prior edge.
