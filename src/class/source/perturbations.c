...
/*
 * MODEL176PX_PT_HELPER_V7
 *
 * This helper is local to perturbations.c.
 * No public CLASS header modification is required.
 */

static int
model176px_pt_active(
    ...
    for (index_k=0; index_k<ppt->k_size[index_md]; index_k++) {
      ...
      if (output_format == class_format) {
        ...
        if (ppt->has_density_transfers == _TRUE_) {
          ...
          /* MODEL178_DESI_BAEST_TRANSFER_V1 */
          class_store_double(
            dataptr,
            tk[ppt->index_tp_delta_cb],
            ((pba->has_aest == _TRUE_) &&
             (ppt->has_source_delta_cb == _TRUE_)),
            storeidx
          );
          /* END MODEL178_DESI_BAEST_TRANSFER_V1 */
...
int perturbations_init(
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    class_test(
               (model176px_pt_active(pba) == _FALSE_)
               &&
               (w_fld_ini >= 0.),
               ppt->error_message,
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        /* MODEL178_DESI_BAEST_TRANSFER_V1
           delta_cb is the pre-ncdm matter combination.
           With omega_cdm=0 and AeST inserted into the matter
           accumulators before the cb snapshot, this is b+AeST. */
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        /* MODEL178_DESI_BAEST_TRANSFER_V1
           Expose the pre-ncdm b+AeST momentum velocity. */
        if (pba->has_aest == _TRUE_)
          ppt->has_source_theta_cb = _TRUE_;
        /* END MODEL178_DESI_BAEST_TRANSFER_V1 */
...
int perturbations_prepare_k_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* MODEL177_AEST_PERTURBATIONS_P2 */
      class_store_columntitle(
        ppt->scalar_titles,
        "delta_aest",
        pba->has_aest
      ...
      /* END MODEL177_AEST_PERTURBATIONS_P2 */
...
int perturbations_vector_init(
  ...
  if (_scalars_) {
    ...
    class_test(
      ...
      (ppt->gauge != newtonian),
      ppt->error_message,
      "MODEL177 AeST perturbations require Newtonian gauge."
    );

    class_define_index(
      ppv->index_pt_delta_aest,
      pba->has_aest,
      index_pt,
      1
    );
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_aest == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_aest] =
          ppw->pv->y[ppw->pv->index_pt_delta_aest];

        ppv->y[ppv->index_pt_theta_aest] =
          ppw->pv->y[ppw->pv->index_pt_theta_aest];

        ppv->y[ppv->index_pt_chi_aest] =
          ppw->pv->y[ppw->pv->index_pt_chi_aest];

        ppv->y[ppv->index_pt_E_aest] =
          ppw->pv->y[ppw->pv->index_pt_E_aest];
      }
      /* END MODEL177_AEST_PERTURBATIONS_P2 */
...
int perturbations_initial_conditions(struct precision * ppr,
  ...
  /* END MODEL177_AEST_PERTURBATIONS_P2 */
    ...
    /*
   * MODEL176PX approaches w=cs2=1/3 at sufficiently early
   * time. Include its density in the total radiation budget
   * used for adiabatic metric initial conditions.
   *
   * It carries no free-streaming shear.
   */
  if (
      pba->has_fld == _TRUE_
      &&
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_aest == _TRUE_) {
        ...
        /*
          Adiabatic density condition:

          delta_A/(1+w_A) = 3/4 delta_gamma.

          The regular synchronous-form dust velocity is zero.
          chi=E=0 is the analytic regular AeST extra-field seed.
        */

        ppw->pv->y[
          ppw->pv->index_pt_delta_aest
        ] =
          (1.0+w_aest_ini)
          *
          3.0/4.0
          *
          ppw->pv->y[
            ppw->pv->index_pt_delta_g
          ];

        ppw->pv->y[
          ppw->pv->index_pt_theta_aest
        ] = 0.0;

        ppw->pv->y[
          ppw->pv->index_pt_chi_aest
        ] = 0.0;

        ppw->pv->y[
          ppw->pv->index_pt_E_aest
        ] = 0.0;
      }
      /* END MODEL177_AEST_PERTURBATIONS_P2 */
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ppw->pv->y[ppw->pv->index_pt_delta_fld] = - ktau_two/4.*(1.+w_fld)*(4.-3.*model176px_pt_cs2(pba,a))/(4.-6.*w_fld+3.*model176px_pt_cs2(pba,a)) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC: curvature

          ppw->pv->y[ppw->pv->index_pt_theta_fld] = - k*ktau_three/4.*model176px_pt_cs2(pba,a)/(4.-6.*w_fld+3.*model176px_pt_cs2(pba,a)) * ppr->curvature_ini * s2_squared; /* from 1004.5509 */ //TBC:curvature
        }
  ...
  if (_scalars_) {
    ...
    if (pba->has_aest == _TRUE_) {
      ...
      class_test(
        (!isfinite(Pi_aest)),
        ppt->error_message,
        "MODEL177 AeST non-finite Pi: "
        "a=%e k=%e rho=%e delta=%e chi=%e E=%e",
        a,
        k,
        rho_aest,
        delta_aest,
    ...
    */
    ...
    /* MODEL184 Native VCDM stress-energy background contribution */
    if (pba->has_vcdm_native == _TRUE_) {
      double rho_vcdm_val, p_vcdm_val, dpdlna_vcdm_val;
      model184_vcdm_background_eval(pba, a, &rho_vcdm_val, &p_vcdm_val, &dpdlna_vcdm_val);
      ppw->rho_plus_p_tot += (rho_vcdm_val + p_vcdm_val);
    }
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_) {
        ...
        ppw->delta_p_fld = model176px_pt_cs2(pba,a) * ppw->delta_rho_fld + (model176px_pt_cs2(pba,a)-ca2_fld)*(3*a_prime_over_a*ppw->rho_plus_p_theta_fld/k/k);
      }
      else {
        s2sq = ppw->s_l[2]*ppw->s_l[2];
        c_gamma_k_H_square = pow(pba->c_gamma_over_c_fld*k/a_prime_over_a,2)*model176px_pt_cs2(pba,a);
...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if (pba->has_aest == _TRUE_) {
      ...
      double delta_aest =
        y[
          pv->index_pt_delta_aest
        ];

      double theta_aest =
        y[
          pv->index_pt_theta_aest
        ];

      double chi_aest =
        y[
          pv->index_pt_chi_aest
        ];
      ...
      /*
       * MODEL184 B4C1C R2 exact-current helper.
       *
       * B4C1B1 proved:
       *
       *   FY = 2 K2 Z0 (x0/a^3) / Q
       ...
       *   Q FY = I0/a^3 = KQ.
       *
       * We audit the identity numerically, but we do NOT
       * subtract KQ-QFY in the physical equation because at
       * very early times both terms are individually enormous.
       * The exact analytic cancellation is imposed directly.
       */
      ...
      double r2_current_identity_scale =
        fmax(
          fmax(
            fabs(KQ_aest),
            fabs(QFY_aest_r2)
      ...
      double r2_current_identity_rel =
        fabs(
          KQ_aest
          -
          QFY_aest_r2
        ...
        /
        r2_current_identity_scale;

      class_test(
        (!isfinite(FY_aest_r2))
        ||
        (!isfinite(QFY_aest_r2))
        ||
        (!isfinite(r2_current_identity_rel))
        ||
        (r2_current_identity_rel > 5.e-10),
        ppt->error_message,
        "MODEL184 B4C1C R2 current identity failure: "
        "a=%e Q=%e FY=%e QFY=%e KQ=%e rel=%e",
        a,
        Q_aest,
        FY_aest_r2,
        QFY_aest_r2,
        KQ_aest,
        r2_current_identity_rel
      );
      ...
      class_test(
        one_plus_w <= 0.0,
        ppt->error_message,
        "MODEL177 AeST invalid 1+w=%e at a=%e",
        one_plus_w,
        a
      );
      ...
      /*
        Published continuity equation converted to conformal
        time and CLASS velocity-divergence convention.
      */
      ...
      /*
        Published Euler equation.

        theta_CLASS = k^2 theta_paper / a.
      */
      ...
      /*
        Closed chi equation derived from:

        chi = varphi + Q alpha
        theta_paper = varphi/Q
        E = alpha_dot + Psi.
      */
      ...
      /*
        Published E equation converted to conformal time.
      */

      {
        ...
        dy[
          ...
          *
          ...
          +
          (
            0.0 /* MODEL184 B4C1C: exact (KQ-Q*FY)*chi cancellation */
            -
            (2.0-pba->aest_KB)
            *
            E_bracket
      ...
      class_test(
        (!isfinite(
          dy[pv->index_pt_delta_aest]
        ))
        ||
        (!isfinite(
          dy[pv->index_pt_theta_aest]
        ))
        ||
        (!isfinite(
          dy[pv->index_pt_chi_aest]
        ))
        ||
        (!isfinite(
          dy[pv->index_pt_E_aest]
        )),
        ppt->error_message,
        "MODEL177 AeST non-finite derivative: "
        "a=%e k=%e delta=%e theta=%e chi=%e E=%e Pi=%e",
        a,
        k,
        delta_aest,
        theta_aest,
