...
#include "model184_vcdm_frozen_table.h"
#include "hi_class.h"


/* ==========================================================
   MODEL177_AEST_BACKGROUND_V1

   Published AeST Cosh background:

     K(Q) = 2 K2 Z0^2 [cosh(Z)-1]
     Z    = (Q-Q0)/Z0
   ...
   Define
     ...
     x = I0/(2 K2 Z0 a^3) = sinh(Z)

   so that

     Q = Q0 + Z0 asinh(x).

   The primary AeST convention is

     8 pi Gtilde rho = Q K_Q - K
     8 pi Gtilde P   = K.

   CLASS stores densities in Friedmann units

     rho_CLASS = (8 pi G rho)/3,

   hence the AeST CLASS contribution used here is

     rho_A = (Q K_Q-K)/3
     p_A   = K/3.

   I0 is derived numerically from Omega0_aest so that

     rho_A(a=1) = Omega0_aest H0^2.

   No perturbation equations are added in this patch.
   ========================================================== */


static double
model177_aest_hypot_minus_one(
  double x
) {
  ...
  /*
    Stable evaluation of sqrt(1+x^2)-1
    without catastrophic cancellation at small x
    and without x*x overflow at large x.
  */
...
static double
model177_aest_rho_class_from_x(
  ...
  hm1 =
    model177_aest_hypot_minus_one(x);
...
static int
model177_aest_background_quantities(
  struct background *pba,
  double a,
  double *rho,
  double *pressure,
  ...
  class_test(
    a <= 0.,
    pba->error_message,
    "MODEL177 AeST called with non-positive a=%e",
    a
  );
  ...
  class_test(
    (!isfinite(x)) || (x <= 0.),
    pba->error_message,
    "MODEL177 AeST invalid x=%e at a=%e",
    x,
    a
  );
  ...
  *pressure =
  ...
  class_test(
    ...
    (!isfinite(*Q)),
    pba->error_message,
    "MODEL177 AeST invalid background at a=%e: "
    "rho=%e p=%e Q=%e x=%e",
    a,
    *rho,
    *pressure,
  ...
  *ca2 =
  ...
  class_test(
    ...
    (!isfinite(*ca2)),
    pba->error_message,
    "MODEL177 AeST invalid w/ca2 at a=%e: "
    "w=%e ca2=%e",
    a,
    *w,
    *ca2
...
model177_aest_normalize(
  ...
  while (
    ...
    class_test(
      ...
      (hi > 1.e100),
      pba->error_message,
      "MODEL177 AeST normalization failed to bracket x0"
    );
  ...
  class_test(
    rho_hi < target,
    pba->error_message,
    "MODEL177 AeST normalization did not bracket "
    "target rho0=%e; rho(hi)=%e hi=%e",
    target,
    rho_hi,
    hi
  );
  ...
  class_test(
    rel > 1.e-12,
    pba->error_message,
    "MODEL177 AeST normalization precision failure: "
    "target=%e got=%e rel=%e",
    target,
    rho0,
    rel
  );
...
/* END MODEL177_AEST_BACKGROUND_V1 */
...
int background_functions(
  ...
  if (pba->has_aest == _TRUE_) {
    ...
    /*
      Since ca2=dP/dRho and
      d rho/d ln a = -3(rho+p),
      dP/d ln a = -3 ca2 (rho+p).
    */
    ...
    /*
      The published benchmark obeys
      w,ca2 < 1e-8.
      Treat it as non-relativistic matter for the
      CLASS matter/equality bookkeeping, as GDM_CLASS
      does for its dustlike generalized matter species.
    */
  ...
  /*
   * MODEL184_NATIVE_VCDM_BACKGROUND
   *
   * Frozen order-3 constrained gravitational background.
   * NOT Lambda/fld/CPL.
   */
  if (pba->has_vcdm_native == _TRUE_) {

    double rho_vcdm_native;
    double p_vcdm_native;
    double dpdlna_vcdm_native;

    class_test(
      model184_vcdm_background_eval(
          pba,
          a,
          &rho_vcdm_native,
          &p_vcdm_native,
          &dpdlna_vcdm_native) != 0,
      pba->error_message,
      "MODEL184 VCDM table evaluation failed at a=%e",
      a
    );

    rho_tot += rho_vcdm_native;
    p_tot += p_vcdm_native;
    dp_dloga += dpdlna_vcdm_native;
  }
...
/*
 * MODEL176PX_BG_HELPER_V7
 *
 * Private sentinel used only in this copied CLASS tree:
 *
 *   fluid_equation_of_state = CLP
 *   w0_fld = -99
 *   wa_fld = epsilon0 > 0
 *
 * Exact MODEL176PX relation:
 *
 *   (y-1) sqrt(y) = C0 a^-3
 *
 *   C0 = epsilon0 sqrt(1+epsilon0)
 */

static int
model176px_bg_active(
    struct background *pba
    ) {
...
int background_w_fld(
  ...
  case EDE:
    ...
    if (pba->has_aest == _TRUE_) Omega_m += pba->Omega0_aest; /* MODEL177_AEST_BACKGROUND_V1 */
...
int background_indices(
  ...
  pba->has_cdm = _FALSE_;
  pba->has_aest = _FALSE_; /* MODEL177_AEST_BACKGROUND_V1 */
  pba->has_idm = _FALSE_;
  ...
  if (pba->Omega0_vcdm_native != 0.)
    pba->has_vcdm_native = _TRUE_;
...
int background_checks(
  ...
  if (pba->has_aest == _TRUE_) {
    ...
    class_test(
      pba->Omega0_aest <= 0.,
      pba->error_message,
      "Omega_aest must be strictly positive when AeST is active"
    );
    ...
    class_test(
      ...
      (pba->aest_KB >= 2.),
      pba->error_message,
      "AeST benchmark requires 0 < aest_KB < 2; got %e",
      pba->aest_KB
    );
    ...
    class_test(
      pba->aest_K2 <= 0.,
      pba->error_message,
      "AeST Cosh requires aest_K2 > 0; got %e",
      pba->aest_K2
    );
    ...
    class_test(
      pba->aest_Q0 <= 0.,
      pba->error_message,
      "AeST Cosh requires aest_Q0 > 0; got %e",
      pba->aest_Q0
    );
    ...
    class_test(
      pba->aest_Z0 <= 0.,
      pba->error_message,
      "AeST Cosh requires aest_Z0 > 0; got %e",
      pba->aest_Z0
    );
  }
  /* END MODEL177_AEST_BACKGROUND_V1 */
  ...
  if (pba->has_fld == _TRUE_) {
    ...
    /*
     * MODEL176PX_BACKGROUND_GUARD_V8
     *
     * Stock CLASS rejects w(a->0) >= 1/3 because its ordinary
     * dark-energy fluid is assumed not to compete with radiation.
     *
     * MODEL176PX is different by construction:
     *
     *   w_X = (y-2)/(3y-2)
     *
     * and, for every finite physical y>1,
     *
     *   1/3 - w_X = 4/[3(3y-2)] > 0.
     *
     * Only the exact mathematical a->0 limit reaches 1/3.
     *
     * Its early abundance has independently been fixed to only
     * 1e-3 of the ordinary radiation density.
     *
     * Therefore exempt only the private MODEL176PX sentinel.
     */
    class_test(
               (model176px_bg_active(pba) == _FALSE_)
               &&
               (w_fld >= 1./3.),
               pba->error_message,
...
int background_initial_conditions(
  ...
  */
  ...
  /*
   * MODEL176PX_INITIAL_RADIATION_ACCOUNTING_V9
   *
   * The stock CLASS radiation-domination sanity check does not
   * classify the generic fld component as radiation.
   *
   * MODEL176PX asymptotes to:
   *
   *   w -> 1/3
   *   cs2 -> 1/3
   *   rho_X/rho_rad -> constant
   *
   * so reducing a_ini cannot remove its finite early fraction.
   *
   * For this sanity check only, include:
   *
   *   Omega_PX_ini = rho_fld/H^2.
   *
   * The physical background itself is unchanged.
   */
  if (
      model176px_bg_active(pba)
      &&
      pba->has_fld == _TRUE_
      ) {
    ...
    printf(
      "MODEL176PX_IC_DOMINATION "
      "Omega_r=%e Omega_de=%e Omega_px=%e "
      "sum=%e\n",
      pvecback[pba->index_bg_Omega_r],
      pvecback[pba->index_bg_Omega_de],
      Omega_px_ini,
      pvecback[pba->index_bg_Omega_r]
      +
      pvecback[pba->index_bg_Omega_de]
...
class_test(fabs(pvecback[pba->index_bg_Omega_r]
             +
             pvecback[pba->index_bg_Omega_de]
             +
             (
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    /* MODEL184_VCDM_BUDGET_REPORT */
    if (pba->has_vcdm_native == _TRUE_) {
      printf(
        "-> %-30s Omega = %-15g , omega = %-15g\n",
        "VCDM grav. acceleration",
        pba->Omega0_vcdm_native,
        pba->Omega0_vcdm_native*pba->h*pba->h
      );
      budget_other += pba->Omega0_vcdm_native;
    }
