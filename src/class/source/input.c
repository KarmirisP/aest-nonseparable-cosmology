...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s_100:
      ...
      break;
    case Omega_dcdmdr:
      Omega_M = ba.Omega0_cdm+ba.Omega0_aest+ba.Omega0_idm+ba.Omega0_dcdmdr+ba.Omega0_b;
      /* *
    ...
    case Omega_ini_dcdm:
      ...
      Omega_M = ba.Omega0_cdm+ba.Omega0_aest+ba.Omega0_idm+Omega0_dcdmdr+ba.Omega0_b;
      gamma = ba.Gamma_dcdm/ba.H0;
      if (gamma < 1)
        a_decay = 1.0;
      else
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** MODEL177_AEST_BACKGROUND_V1:
      AeST effective density and published Cosh parameters */
  ...
  class_test(
    ...
    errmsg,
    "You can only enter one of 'Omega_aest' or 'omega_aest'."
  );

  if (flag1 == _TRUE_) {
    pba->Omega0_aest = param1;
  }
  ...
  class_test(
    pba->Omega0_aest < 0.,
    errmsg,
    "Omega_aest must be non-negative."
  );
  ...
  if (has_m_budget == _TRUE_) {
    ...
    /* MODEL177_AEST_BACKGROUND_V1 */
    class_test(
      Omega_m_remaining < pba->Omega0_aest,
      errmsg,
      "Too much AeST density for requested Omega_m: "
      "remaining=%e Omega_aest=%e",
      Omega_m_remaining,
      pba->Omega0_aest
    );
  ...
  /* MODEL184_VCDM_INPUT */
  int flag_vcdm = _FALSE_;
  double param_vcdm = 0.;
  class_call(parser_read_double(pfc,"Omega_vcdm_native",&param_vcdm,&flag_vcdm,errmsg),
             errmsg,
             errmsg);
  if (flag_vcdm == _TRUE_) {
    pba->Omega0_vcdm_native = param_vcdm;
  }
  ...
  class_test((flag_vcdm == _FALSE_ && pba->Omega0_vcdm_native == 0.) && (flag1 == _TRUE_) && (flag2 == _TRUE_) && ((flag3 == _FALSE_) || (param3 >= 0.)) && (flag4 == _FALSE_),
             errmsg,
             "'Omega_Lambda' or 'Omega_fld' must be left unspecified, except if 'Omega_scf' is set and < 0.");
  class_test((flag_vcdm == _FALSE_ && pba->Omega0_vcdm_native == 0.) && (flag1 == _TRUE_) && (flag2 == _TRUE_) && (flag3 == _FALSE_) && ((flag4 == _FALSE_) || (param4 >= 0.)),
           errmsg,
  ...
  /* MODEL184_VCDM_DYNAMIC_SHAPE_INPUT */
  pba->vcdm_order = 3;
  pba->vcdm_s1 = 4.3390577901297920;
  pba->vcdm_s2 = -5.5743471278619540;
  pba->vcdm_s3 = 2.2352893377321630;
  pba->vcdm_s4 = 0.0;
  pba->vcdm_s5 = 0.0;
  pba->vcdm_s6 = 0.0;

  class_read_int("vcdm_order", pba->vcdm_order);
  class_read_double("vcdm_s1", pba->vcdm_s1);
  class_read_double("vcdm_s2", pba->vcdm_s2);
  class_read_double("vcdm_s3", pba->vcdm_s3);
  class_read_double("vcdm_s4", pba->vcdm_s4);
  class_read_double("vcdm_s5", pba->vcdm_s5);
  class_read_double("vcdm_s6", pba->vcdm_s6);

  class_test(
      pba->Omega0_vcdm_native < 0.,
      errmsg,
      "Omega_vcdm_native must be non-negative.");
...
/* ** BUDGET EQUATION ** -> Add your species here */
  ...
  Omega_tot += pba->Omega0_aest;
  Omega_tot += pba->Omega0_vcdm_native; /* MODEL184 */ /* MODEL177_AEST_BACKGROUND_V1 */
  ...
  /* Step 2 */
  if ((flag1 == _TRUE_) && (param1 == 0.) && (flag2 == _TRUE_) && (param2 == 0.) && (flag_vcdm == _FALSE_)) {
    /* Fill with native VCDM acceleration */
    pba->Omega0_vcdm_native = 1. - pba->Omega0_k - Omega_tot;
    Omega_tot += pba->Omega0_vcdm_native;
    if (input_verbose > 0){
      printf(" -> matched budget equations by adjusting Omega_vcdm_native = %g\n",pba->Omega0_vcdm_native);
    }
  }
...
int input_default_params(struct background *pba,
  ...
  pba->Omega0_fld = 0.;
  pba->Omega0_scf = 0.;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_aest-pba->Omega0_vcdm_native-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr - pba->Omega0_idr -pba->Omega0_idm;
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
